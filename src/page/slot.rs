    ...
    C: cfg::Config,
{
    ...
    #[inline(always)]
    pub(crate) fn get(&self, r#gen: Generation<C>) -> Option<Guard<T, C>> {
        ...
        loop {
            ...
            if r#gen != current_gen || state != Lifecycle::PRESENT {
    ...
    fn mark_release(&self, r#gen: Generation<C>) -> Option<bool> {
        let mut lifecycle = self.lifecycle.load(Ordering::Acquire);
        let mut curr_gen;
        ...
        loop {
            ...
            test_println!(
                "-> mark_release; r#gen={:?}; current_gen={:?};",
                r#gen,
                curr_gen
            );
    ...
    fn release_with<F, M, R>(&self, r#gen: Generation<C>, offset: usize, free: &F, mutator: M) -> R
    where
        F: FreeList<C>,
    ...
    pub(crate) fn init(&self) -> Option<InitGuard<T, C>> {
        ...
        let r#gen = LifecycleGen::<C>::from_packed(lifecycle).0;
        let refs = RefCount::<C>::from_packed(lifecycle);

        test_println!(
            "-> initialize_state; state={:?}; r#gen={:?}; refs={:?};",
    ...
    C: cfg::Config,
{
    ...
    pub(crate) fn insert(&self, value: &mut Option<T>) -> Option<Generation<C>> {
        ...
        test_println!("-> inserted at {:?}", r#gen);
    ...
    pub(super) fn try_remove_value<F: FreeList<C>>(
        ...
        let should_remove = match self.mark_release(r#gen) {
        ...
        if should_remove {
            ...
            self.remove_value(r#gen, offset, free);
        }
    ...
    pub(super) fn remove_value<F: FreeList<C>>(
        ...
        self.release_with(r#gen, offset, free, |item| item.and_then(Option::take))
    }
}
    ...
    C: cfg::Config,
{
    ...
    pub(super) fn try_clear_storage<F: FreeList<C>>(
        ...
        let should_clear = match self.mark_release(r#gen) {
            // If `mark_release` returns `Some`, a value exists at this
            // generation. The bool inside this option indicates whether or not
        ...
        if should_clear {
            ...
            return self.clear_storage(r#gen, offset, free);
        }
    ...
    pub(super) fn clear_storage<F: FreeList<C>>(
        ...
        self.release_with(r#gen, offset, free, |item| {
    ...
    fn release(&self) -> bool {
        ...
        loop {
            ...
            let new_lifecycle = if dropping {
                ...
                LifecycleGen(r#gen).pack(State::Removing as usize)
            } else {
            ...
            test_println!(
                "-> drop guard: state={:?}; r#gen={:?}; refs={:?}; lifecycle={:#x}; new_lifecycle={:#x}; dropping={:?}",
                state,
                r#gen,
                refs,
                lifecycle,
...
impl<T, C: cfg::Config> Guard<T, C> {
    ...
    pub(crate) unsafe fn value(&self) -> &T {
        unsafe { self.slot().item.with(|item| &*item) }
    }
}
...
impl<T, C: cfg::Config> InitGuard<T, C> {
    ...
    /// dangle.
    pub(crate) unsafe fn value(&self) -> &T {
        unsafe { self.slot.as_ref().item.with(|val| &*val) }
    }
    ...
    pub(crate) unsafe fn value_mut(&mut self) -> &mut T {
        unsafe { self.slot.as_ref().item.with_mut(|val| &mut *val) }
    }
